...
async def start(message: Message):
    await message.answer('Привет!', reply_markup=kb.shop) # ??????????????????????????????????????????????????????
...
# настройка индексов пицц и десертов Пицца
...
async def pizza_navigation(callback: CallbackQuery):
    ...
    pizzas = [p for p in pizzas if p[4] == 'pizza']
    if not pizzas:
        await callback.message.answer('Нихуя нету(((')
        return
